...
Per-repo orientation for `donaldgifford/champs`. This file is a Go-shaped
...
`champs` is a Go binary maintained as part of the homelab fleet:

- Single binary under `cmd/champs/`; library code under `internal/` (private to
  the module).
- Built into a distroless container via `Dockerfile`; released as multi-arch
  (linux+darwin × amd64+arm64) archives via `goreleaser`.
- Lives on GitHub (`github.com/donaldgifford/champs`); CI and releases run on
  GitHub Actions.
...
- `just build` — `go build -o bin/champs ./cmd/champs`
...
- `just release v0.1.0` — tag + push. CI picks up the `v*` tag and runs
  ...
  entry on GitHub (via `GITHUB_TOKEN`).
...
- `.github/workflows/ci.yml` runs on every push/PR — `just test` + `just lint`.
- Release workflows fire only on `v*` tag push; `goreleaser` consumes
  `.goreleaser.yml` and `GITHUB_TOKEN`.
